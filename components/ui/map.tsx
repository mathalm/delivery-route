...
type MapControlsProps = {
	...
	/**
	 * "Fit" / recenter control. With `coordinates` (2+ points) the map uses
	 * `fitBounds` so everything stays in view; otherwise `flyTo` a single point.
	 */
	centralizeCoordinates?: {
		longitude?: number;
		latitude?: number;
		zoom?: number;
		/** When 2+ points, map fits bounds around all of them (includes route vertices). */
		coordinates?: [number, number][];
		/** Pixel padding inside the map when using fitBounds (default: 48). */
...
function ControlGroup({ children }: { children: React.ReactNode }) {
	return (
		<div className="border-border bg-background [&>button:not(:last-child)]:border-border flex flex-col overflow-hidden rounded-md border shadow-sm [&>button:not(:last-child)]:border-b cursor-pointer">
			{children}
		</div>
	);
}
...
function MapControls({
	...
	const handleCentralize = useCallback(() => {
		if (!map || !centralizeCoordinates) return;
	...
	return (
		...
		>
			...
			{centralizeCoordinates && (
				<ControlGroup>
					<ControlButton onClick={handleCentralize} label="Centralize map">
